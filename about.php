<?php include("header.php");?>
<?php include('db_con.php');?>
<div class="container-fluid">
    <div class="row" id="wrapper">
    <?php require_once'menubar.php';?>
    <div class="col-sm-12">
        <div class="panel panel-default">
        <div class="panel-body">

<center><h1>Department of Electronics & Telecommunication Engineering</h1>
		<b>Founded on: 2012</b></center>
    <hr>

    	<div class="col-sm-6">
    		<b>Degree offers with duration:</b>
    		<ul>
    			<li>Four years program, B.E. (Engg.)</li>
				
    		</ul>
    		<p> Program organization: Two semesters per year
			Undergraduate students intake: 60</p>
			<br>
				<br>
			<b>Resources:</b>
			<ul>
				<li>Number of Faculty Members: 12</li>
				<li>Number of Technical Staff: 03</li>
				<li>Internet: Department wide internet service with Wi-Fi Support</li>
				<li>Teaching-Learning facilities</li><br>
				
			</ul>
			<br>
				<br>
			<b>Admission:</b><br>
			<p>Admission process starts in early July of each year. The local student will be notified through daily newspaper.</p>
    	</div>
    	<div class="col-sm-6">
    		<b>Research Groups:</b><br>
			<ul>
				<li>Computing Lab</li>
				<li>Programming /OS  Lab</li>
				<li>Communication & Networking Lab</li>
				<li>Circuit Lab</li>
				<li>Project Lab</li>
				<li>Database Management System Lab</li>
				<li>Multimedia Lab</li>
				<li>VLSI & Embedded Systems Lab</li>
				<li>Skill Development Lab</li>
				<li>Seminar Library: More than 250 titles with Internet facilities.</li>
			</ul>
			<p> -- All labs and class rooms are equippedwith audio-visual supports</p>
			<br>
			<b>Extra-curriculum activities:</b><br>
			<ul>
				<li>Programming contest</li>
				<li>Project development contest</li>
				<li>Cultural program, debating and sports</li>
				<li>National wide achievements in programming contest and project</li>
				<li>development</li>
			</ul>
			
    	</div>

		</div>
		</div>
	</div>
	</div>
</div>